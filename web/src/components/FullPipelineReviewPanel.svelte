<script lang="ts">
  ...
  type ResultFilter = 'all' | 'wrong' | 'manual' | 'correct';
  ...
  async function loadResults(): Promise<void> {
    ...
    notice = null;
    try {
      const payload = await handleResponse(await fetch('/api/test64/full-pipeline-results'));
      ...
      notice = { kind: 'success', text: `Loaded ${items.length} full-pipeline results.` };
    } catch (err) {
...
<section class="panel recognition-review-panel full-pipeline-review-panel">
  <div class="panel-heading recognition-review-heading">
    <div>
      ...
      <p>Latest test64 upload endpoint run: detector, recognizer, parser, endpoint status, and latency per image.</p>
    </div>
  ...
  <div class="full-pipeline-meta">
    <p class="muted">Source: {source || '-'} · Created: {createdAt || '-'} · Endpoint: {endpoint || '-'}</p>
  </div>
