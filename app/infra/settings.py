...
class Settings(BaseSettings):
    ...
    models_hf_repo_id: str = "atasesli/smartbite-models"
    models_hf_revision: str = "main"
    ...
    mobile_full_image_rotation_detector_rescue_enabled: bool = True
    mobile_full_image_rotation_detector_rescue_max_candidates: int = 1
    mobile_proposal_rescue_backend: str = "rapidocr_ppocrv5"
    mobile_rapidocr_primary_enabled: bool = True
    mobile_rapidocr_primary_mode: str = "gated"
    ...
    mobile_rapidocr_max_candidates: int = 64
    ...
    mobile_rapidocr_gated_max_boxes_accepted: int = 36
    mobile_rapidocr_gated_probe_top_k: int = 8
    mobile_rapidocr_gated_final_top_k: int = 4
    mobile_rapidocr_gated_prefilter_probe_top_k: int = 36
    mobile_rapidocr_gated_min_probe_digits: int = 4
    mobile_rapidocr_gated_min_date_likeness_score: float = 1.0
    ...
    mobile_product_cropper_rescue_enabled: bool = False
    mobile_product_cropper_model_path: Path = Path("models/yolo20n/yolo26s/yolo26s-best.pt")
    mobile_product_cropper_confidence_threshold: float = 0.15
    mobile_product_cropper_imgsz: int = 1024
    mobile_product_cropper_padding_ratio: float = 0.12
    mobile_product_cropper_max_candidates: int = 3
    mobile_legacy_wide_group_rescue_enabled: bool = False
    mobile_strict_evidence_acceptance_enabled: bool = False
    mobile_rapidocr_role_constraint_enabled: bool = False
    mobile_production_anchor_sibling_enabled: bool = True
    mobile_paired_crop_evidence_enabled: bool = False
    mobile_local_group_wide_crop_enabled: bool = True
    mobile_rotation_wide_crop_enabled: bool = False
    mobile_hardcase_recognizer_rescue_enabled: bool = False
    mobile_hardcase_svtrv2_rec_model_name: str = "ch_SVTRv2_rec"
    mobile_hardcase_svtrv2_rec_model_dir: Path = Path(
        "data/svtrv2_hardcase_expdate_finetune-batch64epoch10/smartbite_svtrv2_hardcase_expdate_rec"
    )
    mobile_hardcase_svtrv2_rec_backend: str = "paddle"
    mobile_hardcase_svtrv2_rec_onnx_path: Path | None = None
    ...
    @field_validator("mobile_expiry_detector_imgsz", "mobile_expiry_max_candidates", "mobile_product_cropper_imgsz", "mobile_product_cropper_max_candidates")
    @classmethod
    ...
    @field_validator("mobile_product_cropper_confidence_threshold")
    @classmethod
    def validate_mobile_product_cropper_confidence_threshold(cls, value: float) -> float:
        return max(0.01, min(float(value), 0.99))

    @field_validator("mobile_product_cropper_padding_ratio")
    @classmethod
    def validate_mobile_product_cropper_padding_ratio(cls, value: float) -> float:
        return max(0.0, min(float(value), 0.5))
    ...
    @field_validator("mobile_rapidocr_primary_mode")
    @classmethod
    def validate_mobile_rapidocr_primary_mode(cls, value: str) -> str:
        normalized = value.strip().lower()
        valid = {"gated", "always", "off"}
        if normalized not in valid:
            raise ValueError(f"mobile_rapidocr_primary_mode must be one of {sorted(valid)}")
        return normalized
    ...
    @field_validator(
        "mobile_rapidocr_max_candidates",
        "mobile_rapidocr_max_boxes_accepted",
        "mobile_rapidocr_primary_max_boxes_accepted",
        "mobile_rapidocr_gated_max_boxes_accepted",
        "mobile_rapidocr_gated_probe_top_k",
        "mobile_rapidocr_gated_final_top_k",
        "mobile_rapidocr_gated_prefilter_probe_top_k",
        "mobile_rapidocr_gated_min_probe_digits",
    )
    @classmethod
    ...
    @field_validator("mobile_rapidocr_gated_min_date_likeness_score")
    @classmethod
    def validate_mobile_rapidocr_gated_min_date_likeness_score(cls, value: float) -> float:
