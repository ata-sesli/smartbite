...
class ExpiryDateParser:
    ...
    def parse(self, normalized_text: str, *, reference_date: date | None = None) -> ParsedDateData:
        ...
        candidates.extend(self._extract_mixed_separator_d_m_y(text))
        candidates.extend(self._extract_d_m_yyyy(text))
        candidates.extend(self._extract_d_m_yy(text))
        candidates.extend(self._extract_expiry_d_m_y_with_time(text))
        candidates.extend(self._extract_unambiguous_m_d_y(text))
        candidates.extend(self._extract_d_m_yy_with_common_ocr_month_confusion(text))
        candidates.extend(self._extract_d_m_noise_digit_yy(text))
        candidates.extend(self._extract_noisy_e_as_two_d_m_yyyy(text))
        ...
        candidates.extend(self._extract_slash_mm_yy(text))
    ...
    def _format_specificity(fmt: str) -> int:
        if fmt in {
            ...
            "DD/MM/YYYY_MIXED_SEPARATOR",
            "TRAILING_DD/MM/YYYY",
            "TRAILING_DD/MM/YY",
            "DD/MM/YY_NOISE_DIGIT",
            "NOISY_DD/MM/YYYY_E_AS_TWO",
            "MM/DD/YYYY_UNAMBIGUOUS",
            "MM/DD/YY_UNAMBIGUOUS",
            "DD/MM/YYYY_WITH_TIME",
            "DD/MM/YY_WITH_TIME",
            "DD/MM/Y_WITH_TIME",
        }:
            return 2
        ...
        for month_candidate in trailing_months:
            ...
            cleaned = [
                ...
                if not (
                    ...
                    and not candidate.fmt.endswith("_MIXED_SEPARATOR")
    ...
    def _extract_mixed_separator_d_m_y(self, text: str) -> list[Candidate]:
        out: list[Candidate] = []
        for match in re.finditer(r"(?<![A-Z0-9])(\d{1,2})([/.])(\d{1,2})([/.])(\d{2}|\d{4})(?!\d)", text):
            ...
            if not parsed:
                continue
            fmt = "DD/MM/YY_MIXED_SEPARATOR" if len(y_raw) == 2 else "DD/MM/YYYY_MIXED_SEPARATOR"
            base = 0.82 if len(y_raw) == 2 else 0.90
            out.append(Candidate(parsed, fmt, self._score(text, match.group(0), base), match.group(0), match.span()))
        return out
    ...
    def _extract_unambiguous_m_d_y(self, text: str) -> list[Candidate]:
        out: list[Candidate] = []
        for match in re.finditer(r"(?<!\d)(\d{1,2})/(\d{1,2})/(\d{2}|\d{4})(?!\d)", text):
            m, d = int(match.group(1)), int(match.group(2))
            if m < 1 or m > 12 or d <= 12:
                continue
            y_raw = match.group(3)
            y = _normalize_year(int(y_raw)) if len(y_raw) == 2 else int(y_raw)
            parsed = _safe_date(y, m, d)
            if not parsed:
                continue
            fmt = "MM/DD/YY_UNAMBIGUOUS" if len(y_raw) == 2 else "MM/DD/YYYY_UNAMBIGUOUS"
            base = 0.86 if len(y_raw) == 2 else 0.93
            out.append(Candidate(parsed, fmt, self._score(text, match.group(0), base), match.group(0), match.span()))
        return out

    def _extract_expiry_d_m_y_with_time(self, text: str) -> list[Candidate]:
        out: list[Candidate] = []
        pattern = r"(\d{1,2})([./:-])(\d{1,2})\2(\d{2}|\d{4}|\d)(?:[./:-]?)(\d{1,2})[:.](\d{2})(?!\d)"
        for match in re.finditer(pattern, text):
            if not self._has_expiry_time_context(text, match):
                continue
            ...
            d, m = int(match.group(1)), int(match.group(3))
            y_raw = match.group(4)
            y = 2020 + int(y_raw) if len(y_raw) == 1 else (_normalize_year(int(y_raw)) if len(y_raw) == 2 else int(y_raw))
            parsed = _safe_date(y, m, d)
            if not parsed:
                continue
            if len(y_raw) == 1:
                fmt = "DD/MM/Y_WITH_TIME"
                base = 0.80
            elif len(y_raw) == 2:
                fmt = "DD/MM/YY_WITH_TIME"
                base = 0.90
            else:
                fmt = "DD/MM/YYYY_WITH_TIME"
                base = 0.94
            out.append(Candidate(parsed, fmt, self._score(text, match.group(0), base), match.group(0), match.span()))
        return out

    @staticmethod
    def _has_expiry_time_context(text: str, match: "re.Match[str]") -> bool:
        ...
        return prefix_compact in {"ET", "EY", "EYT"}
    ...
    def _extract_d_m_noise_digit_yy(self, text: str) -> list[Candidate]:
        out: list[Candidate] = []
        pattern = fr"(?<!\d)(\d{{1,2}})({SEPARATOR_PATTERN})(\d{{1,2}})\2\d\2(\d{{2}})(?!\d)"
        for match in re.finditer(pattern, text):
            d, m = int(match.group(1)), int(match.group(3))
            y = _normalize_year(int(match.group(4)))
            parsed = _safe_date(y, m, d)
            ...
            out.append(
                Candidate(
                    parsed,
                    "DD/MM/YY_NOISE_DIGIT",
                    self._score(text, match.group(0), 0.86),
                    match.group(0),
                    match.span(),
                )
            )
    ...
    def _extract_noisy_e_as_two_d_m_yyyy(self, text: str) -> list[Candidate]:
        out: list[Candidate] = []
        pattern = fr"(?<!\d)\d?2E(\d)({SEPARATOR_PATTERN})(\d{{1,2}})\2(\d{{4}})(?!\d)"
        for match in re.finditer(pattern, text):
            d, m, y = int(f"2{match.group(1)}"), int(match.group(3)), int(match.group(4))
            parsed = _safe_date(y, m, d)
            if not parsed:
                continue
            out.append(
                Candidate(
                    parsed,
                    "NOISY_DD/MM/YYYY_E_AS_TWO",
                    self._score(text, match.group(0), 0.84),
                    match.group(0),
                    match.span(),
                )
            )
    ...
    def _extract_slash_mm_yy(self, text: str) -> list[Candidate]:
        out: list[Candidate] = []
        match = re.fullmatch(r"\s*(0?[1-9]|1[0-2])/(\d{2})\s*", text)
        if match:
            m = int(match.group(1))
            ...
            if parsed:
                out.append(
                    Candidate(
                        parsed,
                        "MM/YY",
                        self._score(text, match.group(0), 0.74),
                        match.group(0),
                        match.span(),
                        "month",
                    )
