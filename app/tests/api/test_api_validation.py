...
from app.api.mobile_expiry import _log_mobile_scan_result
...
def test_mobile_expiry_scan_result_log_includes_result_fields(monkeypatch) -> None:
    captured: dict[str, object] = {}
    captured_console: dict[str, object] = {}

    def fake_info(message: str, *, extra: dict[str, object]) -> None:
        captured["message"] = message
        captured["extra"] = extra

    def fake_console_info(message: str, payload_json: str) -> None:
        captured_console["message"] = message
        captured_console["payload"] = json.loads(payload_json)

    monkeypatch.setattr("app.api.mobile_expiry.logger.info", fake_info)
    monkeypatch.setattr("app.api.mobile_expiry.console_logger.info", fake_console_info)

    row = SimpleNamespace(
        id=uuid4(),
        status="parsed_success",
        corrected_expiry_date=None,
        detected_expiry_date=date(2027, 7, 23),
        raw_text="23.07.2027",
        normalized_text="23.07.2027",
        recognition_confidence=0.91,
    ...
    result = MobileExpiryPipelineResult(
        status="parsed_success",
        detected_expiry_date=date(2027, 7, 23),
        raw_text="23.07.2027",
        normalized_text="23.07.2027",
    ...
    assert captured["message"] == "mobile expiry scan completed"
    extra = captured["extra"]
    assert extra["scan_id"] == str(row.id)
    assert extra["status"] == "parsed_success"
    assert extra["expiry_date"] == "2027-07-23"
    assert extra["detected_expiry_date"] == "2027-07-23"
    assert extra["raw_text"] == "23.07.2027"
    assert extra["normalized_text"] == "23.07.2027"
    assert extra["recognition_confidence"] == 0.91
    assert extra["detector_confidence"] == 0.82
    assert extra["reason"] == "DMY;rotation=original"
    assert extra["elapsed_ms"] == 12
    assert captured_console["message"] == "mobile expiry scan completed %s"
    assert captured_console["payload"] == extra
...
def test_test64_full_pipeline_results_prefers_latest_mobile_direct_report(
    monkeypatch, tmp_path: Path
) -> None:
    root = tmp_path / "onnx_parity"
    upload_dir = root / "test64_mobile_upload_20260510T082438Z"
    run_dir = root / "test64_mobile_direct_20260511T082438Z"
    upload_dir.mkdir(parents=True)
    run_dir.mkdir(parents=True)
    (upload_dir / "mobile_test64_upload_report.json").write_text(
        json.dumps({"summary": {"total": 27}, "rows": []}),
        encoding="utf-8",
    )
    (run_dir / "mobile_test64_direct_report.json").write_text(
        json.dumps(
            {
    ...
    payload = response.json()
    assert payload["run_id"] == run_dir.name
    assert payload["source"] == "mobile-direct-benchmark"
...
def test_mobile_oracle_forensic_audit_endpoint_reads_latest_report(monkeypatch, tmp_path: Path) -> None:
    root = tmp_path / "onnx_parity"
    run_dir = root / "mobile_oracle_forensic_audit_20260518T172344Z"
    crop_dir = run_dir / "crops" / "sample"
    crop_dir.mkdir(parents=True)
    crop_path = crop_dir / "final_01_crop_01.png"
    crop_path.write_bytes(b"fake-png")
    (run_dir / "mobile_oracle_forensic_audit_report.json").write_text(
        json.dumps(
            {
                "created_at": "2026-05-18T17:23:44Z",
                "stable_report": "stable.json",
                "truth_manifest": "truth.json",
                "images_dir": "test64",
                "summary": {
                    "total": 1,
                    "class_counts": {"C_crop_recognition": 1},
                    "selector_can_be_fixed_safely": [],
                    "recognizer_or_data_blocker": ["sample.jpg"],
                },
                "rows": [
                    {
                        "filename": "sample.jpg",
                        "class": "C_crop_recognition",
                        "expected": "2027-05-12",
                        "detected": None,
                        "truth": {"bbox_xyxy": [1, 2, 30, 16]},
                        "selected": {"final_recognition_bbox_xyxy": [3, 4, 24, 12]},
                        "final_candidates": [
                            {
                                "candidate_id": "cand_1",
                                "final_crops": [
    ...
    app = create_app()
    with TestClient(app=app) as client:
        response = client.get("/test64/mobile-oracle-forensic-audit")

    assert response.status_code == 200
    payload = response.json()
    assert payload["run_id"] == run_dir.name
    assert payload["summary"]["class_counts"]["C_crop_recognition"] == 1
    assert payload["items"][0]["image_url"] == "/test64/images/sample.jpg"
    crop = payload["items"][0]["final_candidates"][0]["final_crops"][0]
    assert crop["crop_url"].startswith("/test64/mobile-oracle-forensic-audit/assets")
    assert f"run_id={run_dir.name}" in crop["crop_url"]


def test_mobile_oracle_forensic_audit_asset_endpoint_rejects_path_escape(monkeypatch, tmp_path: Path) -> None:
    root = tmp_path / "onnx_parity"
    run_dir = root / "mobile_oracle_forensic_audit_20260518T172344Z"
    run_dir.mkdir(parents=True)
    monkeypatch.setattr("app.domain.services.TEST64_MOBILE_UPLOAD_ARTIFACTS_DIR", root)
    ...
    with TestClient(app=app) as client:
        response = client.get(
            f"/test64/mobile-oracle-forensic-audit/assets?run_id={run_dir.name}&path=../secret.png"
        )
