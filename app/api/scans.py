...
@get("/test64/mobile-oracle-forensic-audit")
async def get_test64_mobile_oracle_forensic_audit(request: Request, session: AsyncSession) -> dict[str, object]:
    service = _scan_service(request, session)
    try:
        return await service.get_test64_mobile_oracle_forensic_audit()
    except ValidationError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
@get("/test64/mobile-oracle-forensic-audit/assets")
async def get_mobile_oracle_forensic_audit_asset(
    request: Request,
    session: AsyncSession,
    run_id: str,
    path: str,
    ...
    service = _scan_service(request, session)
    try:
        asset_path = service.get_mobile_oracle_forensic_asset_path(run_id=run_id, relative_path=path)
    except ValidationError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
